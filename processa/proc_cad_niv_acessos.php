...
if ($SendCadNivAcesso) {
    ...
    if (!$dados_validos) {
        $erro = true;
        $_SESSION['msg'] = "<div class='alert alert-warning'><i class='fa fa-exclamation-circle' aria-hidden='true'></i>&nbsp;&nbsp; Necessário preencher todos os campos para cadastrar o nível de acesso!</div>";
    }
        ...
        if (($resultado_niv_acesso) AND ( $resultado_niv_acesso->num_rows != 0)) {
            $erro = true;
            $_SESSION['msg'] = "<div class='alert alert-warning'><i class='fa fa-exclamation-circle' aria-hidden='true'></i>&nbsp;&nbsp; Este nível de acesso já está cadastrado!</div>";
        }
    }
    ...
    if ($erro) {
        ...
        if (mysqli_insert_id($conn)) {
            ...
            $_SESSION['msg'] = "<div class='alert alert-success'><i class='fa fa-check-circle' aria-hidden='true'></i>&nbsp;&nbsp; Nível de Acesso cadastrado com sucesso</div>";
            $url_destino = pg . "/listar/list_niv_acessos";
            header("Location: $url_destino");
        } else {
            $_SESSION['msg'] = "<div class='alert alert-danger'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i>&nbsp;&nbsp; Erro ao cadastrar o nível de acesso</div>";
            $url_destino = pg . "/listar/list_niv_acessos";
            header("Location: $url_destino");
        }
    }
} else {
    $_SESSION['msg'] = "<div class='alert alert-danger'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i>&nbsp;&nbsp; Erro ao carregar a página</div>";
    $url_destino = pg . "/listar/list_niv_acessos";
    header("Location: $url_destino");
}
