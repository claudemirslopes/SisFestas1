...
if ($SendCadPagina) {
    ...
    if (!$dados_validos) {
        ...
        if (($resultado_paginas) AND ( $resultado_paginas->num_rows != 0)) {
            $erro = true;
            $_SESSION['msg'] = "<div class='alert alert-warning'><i class='fa fa-exclamation-circle' aria-hidden='true'></i>&nbsp;&nbsp; Este endereço já está cadastrado!</div>";
        }
    }
    ...
    if ($erro) {
        ...
        if (mysqli_insert_id($conn)) {
            ...
            $_SESSION['msg'] = "<div class='alert alert-success'><i class='fa fa-check-circle' aria-hidden='true'></i>&nbsp;&nbsp; Página cadastrada com sucesso!</div>";
            $url_destino = pg . "/listar/list_pagina";
            header("Location: $url_destino");
        } else {
            $_SESSION['msg'] = "<div class='alert alert-danger'><i class='fa fa-exclamation-triangle' aria-hidden='true'></i>&nbsp;&nbsp; Erro ao cadastrar a página</div>";
            $url_destino = pg . "/listar/list_pagina";
            header("Location: $url_destino");
        }
    }
